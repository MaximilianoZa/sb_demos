...
#define PY_OBJECT_TOSTRING(obj)                                                \
  strdup(PyBytes_AsString(PyUnicode_AsUTF8String(obj)))
...
static gre_io_serialized_data_t *create_event_from_array(int argc,
                                                         char **argv) {
  ...
  if (index < argc) {
    format_buffer = greio_array_to_event_data(&argv[index], argc - index,
  ...
  buffer = gre_io_serialize(NULL, target_buffer, name_buffer, format_buffer,
...
/*
see :
https://support.cranksoftware.com/hc/en-us/articles/360039999352-Sending-Events-to-a-Storyboard-Application
...
static PyObject *method_send_event(PyObject *self, PyObject *args) {
  ...
  PyObject *event_channel, *event_target, *event_data, *data_fmt_key,
      *data_value;
  ...
  if (TupleSize != 3) {
    if (!PyErr_Occurred())
      PyErr_SetString(
          PyExc_TypeError,
          "Error! Bad number of arguments - Usage: sbio.send_event( string "
          "channel, string target, dict my_event )");
    return NULL;
  }
  ...
  if (event_channel == NULL || PyUnicode_Check(event_channel) != 1) {
    PyErr_SetString(
        PyExc_TypeError,
        "Error! Bad or missing string argument 'channel'. Usage: "
        "sbio.send_event( string channel, string target, dict my_event )");
    return NULL;
  } else {
    channel_name = PY_OBJECT_TOSTRING(
        event_channel); // Note: PY_OBJECT_TOSTRING uses strdup which uses
                        // malloc so requires freeing
  }
  ...
  if (event_target == NULL || PyUnicode_Check(event_target) != 1) {
    PyErr_SetString(
        PyExc_TypeError,
        "Error! Bad or missing string argument 'target'. Usage: "
        "sbio.send_event( string channel, string target, dict my_event )");
    return NULL;
  } else {
    event_target_name = PY_OBJECT_TOSTRING(
        event_target); // Note: PY_OBJECT_TOSTRING uses strdup which uses malloc
                       // so requires freeing
  }
  ...
  if (event_dict == NULL || !PyDict_Check(event_dict)) {
    PyErr_SetString(PyExc_TypeError,
                    "Error! Invalid argument -  dict my_event");
    return NULL;
  }

  /* there *should* only be one top level key-value dictionary entry in the
     event which is the name of the event and a dictionary containing key-value
     pairs though it is feasible to support multiple events in a dictionary to
     send in a sequence
          */
  while (PyDict_Next(event_dict, &pos_event, &key, &event_data)) {
    ...
    if (key == NULL || PyUnicode_Check(key) != 1) {
      PyErr_SetString(
          PyExc_TypeError,
          "Error! Bad or missing string argument 'event' in ductionary. Usage: "
          "sbio.send_event( string channel, dict my_event )");
      Py_DECREF(key);
      return NULL;
    } else {
      event_name =
          PY_OBJECT_TOSTRING(key); // Note: PY_OBJECT_TOSTRING uses strdup which
                                   // uses malloc so requires freeing
    }

    argc = 0;
    argv = (char **)malloc(sizeof(char *) * 2);
    argv[argc++] = strdup(
        event_target_name); // use a copy as we re-use this for each event
    argv[argc++] = event_name;

    /* do something with the values... */
    if (event_data == NULL || !PyDict_Check(event_data) ||
        num_payload_vars == 0) {
      // No event payload = no problem!
    } else {
      pos_data = 0;
      // Reallocate array for the event payload again starting with event target
      // and event name already in-place where each parameter is 2 array entries
      // - format and value
      argv =
          (char **)realloc(argv, sizeof(char *) * ((num_payload_vars * 2) + 2));
      if (argv == NULL) {
        PyErr_SetString(PyExc_TypeError,
                        "Error! Reallocation of array for payload failed.)");
        return NULL;
      }

      while (PyDict_Next(event_data, &pos_data, &data_fmt_key, &data_value)) {

        argv[argc] = PY_OBJECT_TOSTRING(
            data_fmt_key); // Note: PY_OBJECT_TOSTRING uses strdup which uses
                           // malloc so requires freeing
        argc++;
        if (data_value != NULL) {
          if (PyFloat_Check(data_value)) {
            ...
            argv[argc] = strdup(
                data_buffer); // Note: strdup uses malloc so requires freeing
            ...
            argv[argc] = PY_OBJECT_TOSTRING(
                data_value); // Note: PY_OBJECT_TOSTRING uses strdup which uses
                             // malloc so requires freeing
          } else {
            argv[argc] =
                strdup("nil"); // Note: strdup uses malloc so requires freeing
          }
          argc++;
        }
      }
...
//
// The channel handle will be cached once opened until explicity closed by a
// call to
//
...
static PyObject *method_receive_event(PyObject *self, PyObject *args) {
  ...
  if (TupleSize != 1) {
    if (!PyErr_Occurred())
      PyErr_SetString(PyExc_TypeError,
                      "Error! Bad number of arguments - Usage: "
                      "sbio.receive_event( string channel )");
    return NULL;
  }
  ...
  Py_BEGIN_ALLOW_THREADS ret = gre_io_receive(recv_handle, &buffer);
  Py_END_ALLOW_THREADS
    ...
    nbytes = gre_io_unserialize(buffer, &event_target_name, &event_name,
                                &event_format, (void **)&event_data);

    event_payload =
        greio_event_data_to_string(event_format, event_data, nbytes);
      ...
      while (*p) {
        ...
        if (*p) {
          ...
          if (*p) {
            ...
            if (strcmp(fmt, "1s0") ==
                0) // it is a string so value delimited by ".."
            {
...
static PyObject *method_close_receive_channel(PyObject *self, PyObject *args) {
  ...
  if (TupleSize != 1) {
    PyErr_SetString(PyExc_TypeError,
                    "Error! Bad number of arguments - Usage: "
                    "sbio.close_receive_channel( string channel )");
    return NULL;
  }
...
static PyMethodDef sbio_methods[] = {
    {"send_event", (PyCFunction)method_send_event, METH_VARARGS,
     "Python interface to send a Stoyboard event using gre_io_send syntax, "
     "with or without a payload"},
    {"receive_event", (PyCFunction)method_receive_event, METH_VARARGS,
     "Python interface which blocks waiting for a Stoyboard event on a "
     "channel, returning a dictionary with payload.  The channel is cached "
     "once opened"},
    {"close_receive_channel", (PyCFunction)method_close_receive_channel,
     METH_VARARGS,
     "Python interface which will explicitly close a previously opened receive "
     "channel"},
...
static void sbio_free() { close_all_receive_handles(); }
...
PyMODINIT_FUNC PyInit_sbio(void) {
